...
export interface IDPModel extends Model {
	getServiceProvider(entityID: string) : Promise<SPProviderConfig>
}

export interface IDPEndPoints extends Endpoints {
	loginRequest: EndPoint | string,
}

export interface IDPProviderConfig extends ProviderConfig {
...
export default class IdentityProvider extends Provider {
    ...
    return metadata.buildIDPMetadata(this.config, (shouldSign === undefined) ? true : shouldSign);
  }

  static parseMetadata(xml: string) : IDPProviderConfig{
